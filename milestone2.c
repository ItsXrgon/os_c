...
{
    return (m->rear_blocked + 1) % QUEUE_SIZE == m->front_blocked;
}
...
void semWait(mutex *m, PCB *pcb)
{
    ...
    {
        // Check if the process is already blocked on this mutex
        for (int i = m->front_blocked; i != (m->rear_blocked + 1) % QUEUE_SIZE; i = (i + 1) % QUEUE_SIZE)
        {
            if (m->blocked_queue[i] == pcb->pid)
            {
                printf("Process %d is already blocked on this mutex\n", pcb->pid);
                return;
            }
        }
